...
# Setup
qinling_output_dir <- "./qinling_subregion_tracing_results"
...
# Region name mapping (Chinese column values to English)
...
# Load bronze samples (relative path)
bronze_results_file <- "./bronze_fcm_results_balanced/Bronze_Artefact_Ore_Source_Predictions_Balanced.csv"
    ...
    if (mcc %in% names(results)) {
      ...
      if (length(mi) > 0) {
        ...
        for (idx in mi) {
          ...
          if (conf >= 0.75) { results$Optimized_Prediction[idx] <- s; tc <- tc + 1 }
        }
      }
    }
  }
